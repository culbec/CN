function [x, n_iter] = my_jacobi(A, b, x0, tolerance, it_max)
    % my_jacobi - implementeaza Metoda lui Jacobi, iterativa, pentru a
    % rezolva sistemul Ax = b
    ...
    % Am ales ca M sa fie o matricea usor inversabila de tip diagonala
    M = diag(diag(A));
    N = M - A;
    ...
    for i=1:it_max
        ...
        if rel_error <= tolerance
            figure;
            grid on;
            semilogy(1:i, conv_curbe(1:i), 'o-', 'LineWidth', 1.5, 'DisplayName', 'Jacobi Method - Convergence')
            xlabel('Iteration no.');
