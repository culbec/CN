function [x, n_iter] = my_SOR(A, b, omega, x0, tolerance, it_max)
    % my_SOR - Successive OverRelaxation
    % A         - matricea sistemului
    % b         - vectorul termenilor liberi
    % omega     - rata de relaxare
    ...
    if nargin < 6, it_max=50; end
    if nargin < 5, tolerance=1e-6; end
    if nargin < 4, x0 = zeros(size(b)); end

    if omega <= 0 || omega >= 2
        error("Relaxation rate should be between (0, 2)!")
    end
    ...
    for i=1:it_max
        ...
        if rel_error <= tolerance
            figure;
            grid on;
            semilogy(1:i, conv_curbe(1:i), 'o-', 'LineWidth', 1.5, 'DisplayName', 'SOR Method - Convergence')
            xlabel('Iteration no.');
